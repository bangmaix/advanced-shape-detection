...
class SimplePreciseDetector:
    def __init__(self):
        ...
        # Precise parameters for petak detection
    ...
    def precise_preprocessing(self, image):
        """Precise preprocessing for petak detection"""
    ...
    def detect_precise_petak(self, image):
        """Detect petak with high precision"""
        processed, gray = self.precise_preprocessing(image)
    ...
    def simple_clustering(self, petak_list, distance_threshold=50):
        ...
        for i, petak in enumerate(petak_list):
            ...
            # Start new cluster
            cluster = [petak_list[i]]
            used.add(i)
            
            # Find nearby petak
            for j, other_petak in enumerate(petak_list):
                if j in used:
                    continue
                
                # Calculate distance between centers
                dist = np.sqrt((petak['center'][0] - other_petak['center'][0])**2 + 
                             (petak['center'][1] - other_petak['center'][1])**2)
                
                if dist <= distance_threshold:
                    cluster.append(other_petak)
                    used.add(j)
    ...
    def analyze_petak_structure(self, petak_list, image_shape):
        ...
        # Simple clustering
        clusters = self.simple_clustering(petak_list)
    ...
    def create_precise_visualization(self, image_path):
        """Create precise visualization with all detected petak"""
        # Read image
        ...
        # Detect precise petak
        petak_list, processed = self.detect_precise_petak(image)
    ...
    def generate_report(self, petak_list, structure_analysis):
    ...
    def save_petak_coordinates(self, petak_list, output_file):
        ...
        for i, petak in enumerate(petak_list):
            ...
            coordinates.append({
                ...
                'corners': [[int(corner[0]), int(corner[1])] for corner in petak['corners']]
            })
...
def main():
    """Main function for precise petak detection"""
    detector = SimplePreciseDetector()
    
    print("🎯 Precise Petak Detection System")
    print("=" * 60)
    ...
    # Detect petak with precision
    result = detector.create_precise_visualization(image_path)
    if result is None:
        return
    ...
    report = detector.generate_report(petak_list, structure_analysis)
